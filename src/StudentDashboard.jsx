...
// ==========================================
// 4. MASTER STUDENT DASHBOARD (ONLY VIDEO HUB NOW)
// ==========================================
export default function StudentDashboard({ user, onBack, onOpenLab, onOpenPyq }) {
  ...
  const goToSlide = (idx) => { if (idx >= 0 && idx < lessonData.length) { setActiveIdx(idx); setRevealedPoints(0); setActiveResource(null); setOverlayZoom(1); }};
  ...
  const getAssetIcon = (type) => { return type === 'svg' ? '🧬' : type === 'youtube' ? '▶️' : type === 'question' ? '❓' : type === 'custom_mcq' ? '📝' : type === 'code' ? '💻' : type === 'mnemonic' ? '🧠' : '🖼️'; };

  useEffect(() => {
    const handleKeyDown = (e) => {
      if (['INPUT', 'TEXTAREA'].includes(document.activeElement?.tagName)) return;
      if (e.key === 'Escape') { setActiveResource(null); setOverlayZoom(1); return; }
  ...
  return (
    <div className="h-screen w-full bg-slate-950 text-slate-200 flex flex-col font-sans overflow-hidden">
      ...
      {/* GLOBAL TOP NAVIGATION WITH MODE TOGGLE */}
      ...
      <div className="flex-1 flex overflow-hidden relative">
            ...
            <div className={`w-[96vw] max-w-[1600px] h-[94vh] flex flex-col pointer-events-auto transition-all duration-300 shadow-[0_0_80px_rgba(0,0,0,0.8)] rounded-2xl overflow-hidden bg-slate-100/95 backdrop-blur-xl border border-slate-300`}>
              ...
              <div className="flex justify-between items-center px-6 py-3 border-b bg-white border-slate-200 shadow-sm shrink-0">
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-black tracking-widest uppercase px-3 py-1 rounded-md bg-slate-100 text-slate-500 border border-slate-200">{getAssetIcon(activeResource.type)} PRESENTATION MODE</span>
                  <span className="text-sm font-bold truncate max-w-md text-slate-800">{activeResource.title || 'Resource Viewing'}</span>
                </div>
                <div className="flex items-center gap-3">
                  {['image', 'svg'].includes(activeResource.type) && (
              ...
              <div className="flex-1 w-full h-full overflow-y-auto custom-scrollbar flex flex-col p-4 md:p-8 relative bg-slate-200/50">
                {activeResource.type === 'youtube' && getYoutubeId(activeResource.payload) ? (
                  <div className="my-auto mx-auto w-full max-w-7xl shrink-0 shadow-2xl rounded-2xl overflow-hidden border border-slate-800 bg-black aspect-video"><iframe className="w-full h-full" src={`https://www.youtube.com/embed/${getYoutubeId(activeResource.payload)}?autoplay=1`} frameBorder="0" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div>
                ) : activeResource.type === 'image' ? (
                  <div className="my-auto mx-auto shrink-0 transition-transform duration-200" style={{ transform: `scale(${overlayZoom})`, transformOrigin: 'top center' }}><img src={activeResource.payload} alt={activeResource.title} className="max-w-full max-h-[85vh] object-contain rounded-xl shadow-2xl border border-slate-300 bg-white" /></div>
                ) : activeResource.type === 'question' ? (
                  <div className="my-auto mx-auto w-full max-w-5xl bg-white p-8 md:p-14 rounded-3xl shadow-2xl border border-slate-200 shrink-0"><PyqInteractiveBlock questionTarget={activeResource.payload} /></div>
                ) : activeResource.type === 'custom_mcq' ? (
                  <div className="my-auto mx-auto w-full max-w-5xl bg-white p-8 md:p-14 rounded-3xl shadow-2xl border border-slate-200 shrink-0"><CustomMcqBlock mcqPayload={activeResource.payload} /></div>
                  ...
                  <div className="my-auto mx-auto w-full max-w-6xl flex flex-col shrink-0 transition-transform" style={{ transform: `scale(${overlayZoom})`, transformOrigin: 'top center' }}>
                     <div className="w-full min-h-[80vh] flex flex-col bg-white rounded-2xl shadow-2xl border border-slate-300 overflow-hidden">
                       ...
                       <iframe srcDoc={getIframeDoc(activeResource.payload)} className="w-full flex-1 min-h-[80vh] border-none bg-white" sandbox="allow-scripts allow-same-origin" />
                     </div>
                  </div>
                ) : activeResource.type === 'mnemonic' ? (
                  <div className="my-auto mx-auto w-full max-w-5xl bg-gradient-to-br from-amber-400 to-orange-600 rounded-[3rem] p-12 md:p-20 shadow-2xl text-center relative overflow-hidden shrink-0">
                    ...
                    <p className="text-4xl md:text-6xl font-black text-white leading-tight drop-shadow-xl">{activeResource.payload}</p>
                  </div>
                ) : activeResource.type === 'code' ? (
                  <div className="my-auto mx-auto w-full max-w-5xl flex flex-col bg-[#0a0f1c] rounded-3xl shadow-2xl border border-slate-700 overflow-hidden shrink-0 max-h-[85vh]">
                      ...
                      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar"><pre className="font-mono text-cyan-400 text-xl md:text-2xl leading-relaxed tracking-wide whitespace-pre-wrap"><code>{activeResource.payload}</code></pre></div>
                  </div>
                          ...
                          <ul className="space-y-6 pl-4">
                            {stageContent.map((item, i) => (
                              <li key={i} className="flex items-start gap-4 text-slate-200">
                                <span className="text-blue-500 text-xl font-black shrink-0 mt-0.5">▪</span><div className="text-lg md:text-xl font-medium leading-relaxed">{item}</div>
                              </li>
                            ))}
                          </ul>
        ...
        <aside className={`bg-slate-900 border-l border-slate-800 flex flex-col shrink-0 overflow-hidden transition-all duration-300 ${rightSidebarOpen ? 'w-80 opacity-100' : 'w-0 opacity-0 border-none'}`}>
          <div className="p-4 border-b border-slate-800 flex items-center justify-between shrink-0">
            ...
            <span className="px-2 py-0.5 bg-slate-800 text-slate-400 rounded text-[9px] font-bold">{interactiveMedia.length + mnemonics.length + codes.length}</span>
          </div>
          {!hasAccess ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center opacity-50"><span className="text-3xl block mb-2">🔒</span><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">Resources locked<br/>with this chapter.</p></div>
          ) : (
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-5">
              ...
              {currentAssets.length === 0 && <div className="text-center py-10 opacity-50"><span className="text-3xl block mb-2">📦</span><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Deck Empty</p></div>}
            </div>
          )}
        </aside>
      </div>
